...
<div class="inside">
	...
	if ( $viewable ) :
		...
		// cambios dic-2019
		$add_from_erp = add_from_erp_html( $post->ID ); 
		/*
	...
	<div id="edit-slug-box" class="hide-if-no-js">
			...
			if ( $has_sample_permalink ) {
				echo $sample_permalink_html.'<br><br>';
				echo $add_from_erp.'<br><br>';
...
function productsErp_modal() {
		?>
		<div id="erp-modal-backdrop" style="display: none"></div>
		<div id="erp-modal-wrap" style="display: none">
			<form id="bait" tabindex="-1"></form>
			<form id="erp-modal" tabindex="-1">
				<div id="erp-modal-title">
					<?php esc_html_e( 'Importar productos desde ERP', 'Importar productos desde ERP' ); ?>
					<button type="button" id="erp-modal-close"><span class="screen-reader-text"><?php esc_html_e( 'Close', 'wpforms-lite' ); ?></span></button>
				</div>
				<div id="erp-modal-inner">

					<div id="erp-modal-options">
						<?php
						echo '<p id="erp-modal-notice">';
						printf(
							wp_kses(
								/* translators: %s - WPForms documentation link. */
								__( 'Esta ventana te permite importar productos desde Openbravo ERP', 'Esta ventana te permite importar productos desde Openbravo ERP' ),
								array(
									'a' => array(
										'href'   => array(),
										'rel'    => array(),
							...
							'http://www.prime-erp.com'
						);
						echo '</p>';
						$args  = apply_filters( 'erp_modal_select', array() );
						//$forms = wpforms()->form->get( '', $args );
						
							printf( '<p><label for="erp-modal-select-form">%s</label></p>', esc_html__( 'Escriba los códigos de los productos separados por comas', 'wpforms-lite' ) );
							echo '<input style="width:100%" id="erp-modal-select-form" placeholder="Ejm: 20010003,20010004,20010005"/>';
							echo '<br>';
...
<div style="display:none" id="loader" class="loader"></div>
				...
				<div class="submitbox">
					<div id="erp-modal-cancel">
						<a class="submitdelete deletion" href="#"><?php esc_html_e( 'Cancel', 'wpforms-lite' ); ?></a>
					</div>
					
						<div id="erp-modal-update">
							<button class="button button-primary" id="erp-modal-submit"><?php esc_html_e( 'Insertar', 'wpforms-lite' ); ?></button>
						</div>
...
@keyframes spin {
  ...
  100% { transform: rotate(360deg); }
			...
			#erp-modal #erp-modal-options {
				padding: 8px 0 12px;
			}

			#erp-modal #erp-modal-options .erp-modal-inline {
				display: inline-block;
				margin: 0;
				padding: 0 20px 0 0;
			}

			#erp-modal-select-form {
				margin-bottom: 1em;
				max-width: 100%;
			}
