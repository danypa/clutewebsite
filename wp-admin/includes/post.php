...
function add_from_erp_html( $id, $new_title = null, $new_slug = null ) {
	...
	// Permalinks without a post/page name placeholder don't have anything to edit
		...
		$return  = '<strong>' . __( 'Códigos asociados al producto:' ) . "</strong>\n";
		
		
		$return .= '<span id="edit-producterp-buttons"><button type="button" class="edit-producterp button button-small hide-if-no-js" aria-label="' . __( 'Editar codigos asociados' ) . '">' . __( 'Seleccionar' ) . "</button></span>\n";
		//$return .= '<span id="editable-post-name-full">' . esc_html( $post_name ) . "</span>\n";
	

	
	
	$return = apply_filters( 'add_from_erp_html', $return, $post->ID, $new_title, $new_slug, $post );
