...
function base64url_encode(str) {
    var utf8str = unescape(encodeURIComponent(str))
    return base64_encode_data(utf8str, utf8str.length, b64u).replace(/=/g,"");
}

	function editProductErp(){
		event.preventDefault();
			$('#erp-modal-backdrop, #erp-modal-wrap').css('display','block');
			$( document.body ).addClass( 'modal-open' );
}

$(document).on('click', '#erp-modal-close, #erp-modal-cancel a', function(event) {
			event.preventDefault();
			erpModalClose();
		});

var erpModalClose = function() {
			if ( $('#erp-modal-select-form').length ) {
				$('#erp-modal-select-form').value = '';

			}
			$('#erp-modal-backdrop, #erp-modal-wrap').css('display','none');
...
$(document).on('click', '#erp-modal-submit', function(event) {
			event.preventDefault();

			var postID = $('#post_ID').val();
			var products = $('#erp-modal-select-form').val(); 			
			var tokens64 = base64url_encode('5245928cww323dm|'+postId+"|"+products);

			$('#loader').css('display','block');
			jQuery.ajax({
			    type: "get",
			    url: 'https://clute.com.pe/ecommerce/index.php/woocommerce/woocommerce_addproducts/'+tokens64,
	...
	function editPermalink2() {
		var i, slug_value,
			...
			permalinkOrig = permalink.html(),
			permalinkInner = $( '#sample-permalink a' ).html(),
			buttons = $('#edit-slug-buttons2'),
			buttonsOrig = buttons.html(),
			full = $('#editable-post-name-full2');
		...
		buttons.children( '.save' ).click( function() {
			...
			if ( new_slug == $('#editable-post-name-full2').text() ) {
				buttons.children('.cancel').click();
				return;
			}
	...
	$( '#titlediv' ).on( 'click', '.edit-slug', function() {
        ...
        $( '#titlediv' ).on( 'click', '.edit-producterp', function() {
		editProductErp();
	});
/*
	$( '#titlediv' ).on( 'click', '.edit-slug2', function() {
		editPermalink2();
	});
*/
	/**
