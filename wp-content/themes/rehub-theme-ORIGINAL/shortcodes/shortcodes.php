...
function wpsm_shortcode_button( $atts, $content = null ) {
    ...
    $icon_show = (!empty($atts['icon'])) ? '<i class="far fa-'.$atts['icon'].'"></i>' : ''; 
...
if(!function_exists('wpsm_titlebox_shortcode')) {
		function wpsm_titlebox_shortcode($atts, $content) {   
			...
			// return the url
		    return '<div class="wpsm-titlebox wpsm_style_' . $style .$themeclass. '"><strong class="'.$colorclass.'">' . $title . '</strong><div>'.$content.'</div></div>';  
		} 
		// add the shortcode to system
}
				...
				<div class="ce_common_simple_list">
			        ...
			        <div class="flowhidden pb10 pt15 border-grey-bottom">               
			            ...
			            <div class="floatright text-right-align disablemobilealign wpsm-one-half-mobile">
			            	<span class="font120 rehub-main-font fontbold wooprice_count"><?php echo ''.$product->get_price_html(); ?>
			            	</span>                      
			            </div>                                            
			        </div>
			        <?php endwhile; ?>					
			            ...
			            <div class="rh_listofferitem rh_list_mbl_im_left border-grey-bottom"> 
			            	<div class="rh-flex-center-align rh-flex-justify-center pt15 pb15 mobileblockdisplay">
				                <div class="rh_listcolumn rh_listcolumn_image text-center">
				                	<?php if ($logo == 'brand') :?>
				                		<?php WPSM_Woohelper::re_show_brand_tax('logoname', '80'); //show brand logo?>
				                ...
				                <div class="rh_listcolumn rh-flex-grow1 rh_listcolumn_text">
				                	<?php if (!$notitle):?>
				                        <a href="<?php echo esc_url($woolink) ?>" class="no-color-link rehub-main-font blockstyle mb10 lineheight15 font90">
				                            <?php the_title(); ?>
				                        </a>
			                    	<?php endif;?>
			                    	<?php if($titlefield):?>
			                    		<div class="no-color-link rehub-main-font blockstyle mb10 font90 lineheight15"><?php echo wpsm_get_custom_value(array('field'=>$titlefield));?></div>
			                    	<?php endif;?>
				                ...
				                <div class="rh_listcolumn rh_listcolumn_price text-center"> 
				                	<?php echo '<span class="price_count fontbold rehub-main-color rehub-main-font">'.$product->get_price_html().'</span>';?>    
				                </div>
...
function wpsm_button_popup_funtion( $atts, $content = null ) {
	...
	$out = '<div class="csspopup" id="custom_popup_rh_'.$rand.'"><div class="csspopupinner"'.$width.'><span class="cpopupclose">×</span>'.do_shortcode($content).'</div></div>';
...
function rehub_exerpt_function( $atts, $content = null ) { 
    ...
    if ($reviewtext =='1'){
    	if($post->post_type == 'product'){
    		...
    		$reviewtext = vp_metabox('rehub_post.review_post.0.review_post_summary_text');		 
    	}  
		if($length){
    ...
    elseif ($reviewheading =='1') {
    	if($post->post_type == 'product'){
    		...
    		$out .= vp_metabox('rehub_post.review_post.0.review_post_heading');		 
    	}    	
	                    ...
	                    <p class="postcont">
	                    	...
	                    	<?php elseif ($module_desc =='review') :?>
	                    		<?php if(get_post_type($post->ID) == 'product'):?>
	                    			<?php $description = get_post_meta($post->ID, '_review_post_summary_text', true);?>
...
function wpsm_cartbox_shortcode( $atts, $content = null ) {
    ...
    if ($design == '2'){
		...
		$output .= '<div class="categoriesbox rh-cartbox rh-heading-hover-color rh-hover-up rh-shadow4">';
		if ($revert_image) :
			if ($image) :
...
if($btntext !='') :
	...
	$output .='<i class="far fa-external-link-square"></i></div></div></div>';
return $output;
}
} 
...
if (is_user_logged_in()) {
    ...
    $output .= '<ul class="user-dropdown-intop-menu">';
        ...
        if (function_exists('bp_core_get_user_domain')) :
			$output .= '<li class="bp-profile-edit-menu-item menu-item"><a href="'.bp_core_get_user_domain( $user_id ).'"><i class="fas fa-cogs"></i></i><span>'. esc_html__("Edit Profile", "rehub-theme") .'</span></a></li>';        	
        endif;
        if ($submit_url) :
        	...
        	$output .= '<li class="user-addsome-link-intop menu-item"><a href="'. esc_url($submit_url) .'"><i class="fas fa-cloud-upload"></i><span>'. $submit_url_label .'</span></a></li>';
        endif; 
        if ($edit_url) :
        	...
        	$output .= '<li class="user-editposts-link-intop menu-item"><a href="'. esc_url($edit_url) .'"><i class="fas fa-edit"></i><span>'. $edit_url_label .'</span></a></li>';
        endif;
        $ifvendor = false;  
        if (defined('wcv_plugin_dir')) :
        	...
        	if (!empty($redirect_to)){
	        	$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. esc_url($redirect_to) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. esc_html__("Manage Your Shop", "rehub-theme") .'</span></a></li>';
	        	$ifvendor = true;        	
        	}
        endif; 
		if( defined( 'WCFMmp_TOKEN' ) && wcfm_is_vendor( $user_id ) ):
			$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. esc_url(get_wcfm_url()) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. esc_html__("Manage Your Shop", "rehub-theme") .'</span></a></li>'; 
			$ifvendor = true;
		endif;        
        if( function_exists('dokan_is_user_seller')) :
        	$is_vendor = dokan_is_user_seller( $user_id );
        	if($is_vendor) :
	        	$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. dokan_get_navigation_url() .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. esc_html__("Manage Your Shop", "rehub-theme") .'</span></a></li>'; 
	        	$ifvendor = true;
	        endif; 
        endif; 
        if( class_exists('WCMp')) :
        	...
        	if($is_vendor) :
        		...
        		if ($dashlink > 0):
	        		$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. get_permalink($dashlink) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. esc_html__("Manage Your Shop", "rehub-theme") .'</span></a></li>'; 
	        	endif; 
	        	$ifvendor = true;        		
    		endif;
        endif; 
        if(class_exists('Woocommerce') && $ifvendor ==false && rehub_option('disable_woo_scripts') != 1){
        	...
        	if(!empty($myaccpageid)){
	        	$output .= '<li class="user-editorders-link-intop menu-item"><a href="'. get_permalink($myaccpageid) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. esc_html__("Manage Your Orders", "rehub-theme") .'</span></a></li>';
        	}         	
        }                                 
        ...
        $output .= '<li class="user-logout-link-intop menu-item"><a href="'. wp_logout_url( home_url()) .'"><i class="far fa-lock-alt"></i><span>'. esc_html__("Log out", "rehub-theme") .'</span></a></li>';
$output .= '</ul></div>';
} else {
...
function wpsm_comparison_button( $atts, $content = null ) {
	...
	$out = '<span';   
    $out .=' class="wpsm-button wpsm-button-new-compare addcompare-id-'.$postid.' '.$atts['color'].' '.$atts['size'].''.$compare_active.$class_show.'" data-addcompare-id="'.$postid.'"><i class="fal re-icon-compare"></i><span class="comparelabel">'.esc_attr($atts['label']).'</span></span>';
    return $out;
}
}
...
function wpsm_get_custom_value($atts){
    ...
    if($result){
    	...
    	if($showtoggle){
    		$out .= '<i class="fas fa-check-circle greencolor"></i>';
    	}else{
    		$out .= $result;
    	}
    	...
    	if($show_empty){
	    	...
	    	if($showtoggle){
	    		$out .= '<i class="far fa-times redcolor"></i>';
	    	}else{
	    		$out .= '-';
	    	}	    	
		    if($list){
...
function wpsm_tax_archive_shortcode( $atts, $content = null ) {
	...
	$args = array( 'hide_empty' => $hide_empty, 'order' => 'ASC', 'taxonomy'=> $taxonomy, 'number'=> $number, 'child_of' => $child_of);
		...
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				...
				if( $term->parent == '0' ){
					...
					$term_titles .= ($thumbnail_url && $leftimage) ? '<a href="'. esc_url(get_term_link($term)) .'" title="'. esc_attr(sprintf( esc_html__('View all post filed under %s', 'rehub-theme' ), $term->name)) .'" class="mr15"><img '.$imgcl.' alt="'. esc_attr($term->name) .'" '.($originalimg) ? '' : 'width="'. $imageheight .'"'.' /></a>' : '';
					...
					if($showcount){
						$term_titles .= '<div class="greycolor mb10 font90">'.$term->count.' '.__('items', 'rehub-theme').'</div>';
					}					
					$term_titles .= '<div class="subcategortes font70 lineheight15 blackcolor">'. $term_childes .''. $ellipsis .'</div>'; 
					$term_titles .= '</div>';					
					if($thumbnail_url && !$leftimage){
	...
	elseif ($type == 'storegrid') {
		if($classcol == 'col_wrap_tenth'){
			$columns = 10;
			$classitem .= ' rh_centered_image';
		}else{
			$columns = 5;
			$term_titles .= "<style scope>.rh-hover-tax-title{height:".$imageheight."px;}.rh-cash-tax img{max-height:".$imageheight."px;}</style>";
		}
		if(!empty($terms) && !is_wp_error($terms)) {
...
function rh_wcv_vendorslist_flat( $atts ) {
		...
		<div class="tabledisplay mb20">
			<form id="search-sellers" role="search" method="get" class="celldisplay search-form floatleft mb10">
				...
				<button type="submit" alt="'. esc_html__('Search', 'rehub-theme') .'" value="'. esc_html__('Search', 'rehub-theme') .'" class="btnsearch"><i class="fal fa-search"></i></button>
			</form>
...
function rh_compare_icon($atts, $content = null ) {
	if (rehub_option('compare_page') != '' || rehub_option('compare_multicats_textarea') != '') {	
		$output = '<span class="re-compare-icon-toggle">';
			$output .= '<i class="fal fa-analytics"></i>';
...
function wpsm_get_bigoffer($atts){
	...
	if($post_id && is_numeric($post_id)){
        ...
        <div class="rh-tabletext-block">
            <?php if(!$notitle):?>
            	<div class="rh-tabletext-block-heading"><h4><a href="<?php echo get_the_permalink($post_id) ?>"><?php echo esc_attr($title); ?></a></h4> </div>
            <?php endif;?>		
	            ...
	            <div class="single_compare_right">	
                    ...
                    <?php if($overall_review):?>
                        ...
                        <div class="bigoffer-overall-score mb20">
                            ...
                            <span class="text-read-review floatright"><a href="<?php echo get_the_permalink($post_id) ?>"><?php esc_html_e('Read review', 'rehub-theme');?></a></span>
                            </div>
        ...
        <div class="<?php echo esc_attr($class);?> modulo-lightbox rh_post_videos mt15 mb20" data-galleryid="rhvid_<?php echo ''.$random_key;?>">   
            <?php foreach($post_image_videos as $key=>$video):?>
                <a href="<?php echo esc_url($video);?>" data-rel="rehub_postvid_gallery_<?php echo ''.$random_key;?>" target="_blank" class="mb10 inlinestyle rh_videothumb_link" data-poster="<?php echo parse_video_url(esc_url($video), 'maxthumb'); ?>" data-thumb="<?php echo parse_video_url(esc_url($video), 'hqthumb'); ?>"> 
					<img src="<?php echo parse_video_url(esc_url($video), 'maxthumb'); ?>" alt="image" />
                </a>                               
            <?php endforeach;?>                       
        </div>
        <?php  wp_enqueue_script('modulobox'); wp_enqueue_style('modulobox');?>
		...
		$output .= '<ul class="buddypress widget">';
			foreach( (array) $group->admins as $admin ) {
				$output .= '<li class="vcard mb15">';
					...
					$output .='<div class="item"><div class="item-title-bpadmin"><a href="'.bp_core_get_user_domain( $admin->user_id, $admin->user_nicename, $admin->user_login ).'">'.$admin->user_nicename.'</a></div><a href="'.$link.'" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. $txt .'</span></a></div>';					
				$output .= '</li>';
			} 
		$output .= '</ul>';
	}
...
function wpsm_reviewbox( $atts, $content = null ) {
    	...
    	if($score){
    		...
    		elseif($compact == 'smallsquare'){
			    $out = '<div class="review-small-square mb10 fontbold text-center whitecolor mr10 floatleft rtlml10 r_score_'. round($score).'">
                    <div class="overall-score">
                        <span class="overall">'.$score.'</span><span class="font70 fontnormal">/10</span>
                    </div> 
                </div> ';
    		} 
    		elseif($compact == 'smallcircle'){
			    $out = '<div class="review-small-circle mb10 fontbold text-center whitecolor mr10 floatleft rtlml10 r_score_'. round($score).'">
                    <div class="overall-score">
                        <span class="overall">'.$score.'</span></span>
                    </div> 
                </div> ';
    		}     		   		   		
    ...
    elseif($woobox){
		...
		$out = '<div class="rate_wide_block mobileblockdisplay rh-flex-center-align '.$cols.'" id="rate_wide_block_'.$rand_id.'">';
			...
			if (!empty($thecriteria[0]['review_post_name']))  {
				$out .= '<div class="rh_col wooratebarwrap rate-line">';
				    foreach ($thecriteria as $criteria) {
				    	if(!empty($criteria)){
	...
	elseif($regular){
		...
		$out = '<div class="rate_bar_wrap"><div class="review-top">';
			$out .= '<div class="overall-score"><span class="overall">'.$total_score.'</span><span class="overall-text">'.__('Expert Score', 'rehub-theme').'</span></div>';
		...
		$out = '<div class="rate_bar_wrap"><div class="review-top"><div class="overall-score">';
			...
			$out .='<div class="review-text"><span class="review-header">'.esc_html($title).'</span><p>'.esc_html($description).'</p></div></div>';
			if (!empty($criterias))  {
				$out .='<div class="review-criteria">';
				    foreach ($thecriteria as $criteria) {
				    	if(!empty($criteria)){
...
function wpsm_banner_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<div id="wpsm_banner_'.$rand_id.'" class="wpsm-banner-wrapper rh-transition-box'.$overlay_class.'">';
	$output .= '<style scope>#wpsm_banner_'.$rand_id.' .wpsm-banner-image{background-color:'.$bg.';'.$b_style.''.$h_style.'}#wpsm_banner_'.$rand_id.' .wpsm-banner-text i{color:'.$color.'}#wpsm_banner_'.$rand_id.' .wpsm-banner-text:before, #wpsm_banner_'.$rand_id.' .wpsm-banner-text:after{border-color:'.$color.';top:'.$b_pad.';right:'.$b_pad.';bottom:'.$b_pad.';left:'.$b_pad.';}#wpsm_banner_'.$rand_id.' .celldisplay{'.$c_pad.'}'.$colortext.$firstsize.$secondsize.$vertical.'</style>';	
		if (!empty($url)) { $output .= '<a href="'.$url.'" '.$target.' title="'.$title.'">'; }
...
if( !function_exists('wpsm_itinerary_item_shortcode') ) {
	function wpsm_itinerary_item_shortcode( $atts , $content = null ) {
		...
		$output = '<div class="wpsm-itinerary-item">';
			$output .= '<div class="wpsm-itinerary-icon"><span style="background-color:'. $color .'"><i class="far '. $icon .'" aria-hidden="true"></i></span></div>';
		...
		$output = '<div class="wpsm-versus-item" id="wpsm-vs-'.$rand_id .'">';
			...
			$output .= '<div class="wpsm-versus-cont">';
				...
				$output .= '<div class="'.implode(' ', $fclass).'">';
					if($firstcolumntype == 'tick'){
						$output .= '<i class="fas fa-check-circle" aria-hidden="true"></i>';
					}
					elseif($firstcolumntype == 'times'){
						$output .= '<i class="far fa-times" aria-hidden="true"></i>';
					}		
					elseif($firstcolumntype == 'image'){
...
if( !function_exists('wpsm_compare_bar_shortcode') ) {
	function wpsm_compare_bar_shortcode( $atts  ) {		
		...
		foreach ($bar_array as $index => $barline) {
			...
			$linkclose = (!empty($link)) ? ' <i class="far fa-external-link" aria-hidden="true"></i></a>' : '';
