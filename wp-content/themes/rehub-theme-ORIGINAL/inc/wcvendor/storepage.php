	    		...
	    		<div id="wcvendor_profile_act_desc" class="wcvendor_profile_cell">
	    			<div class="wcvendor_store_name">
						<?php if ( $verified_vendor ) : ?>	   			
							<div class="wcv-verified-vendor">
								<i class="fas fa-shield-check" aria-hidden="true"></i> <?php echo esc_attr($verified_vendor_label); ?>
							</div>
						<?php endif; ?>	    			
	    			...
	    			<div class="wcvendor_store_desc">
					    <?php if ( class_exists( 'WCVendors_Pro' ) ) :?>
						    ...
						    ?>
							<?php if ($address) : ?>
								<i class="far fa-map-marker-alt"></i> <?php echo esc_html($address); ?>
	    ...
	    <aside class="rh-mini-sidebar user-profile-div floatleft tabletblockdisplay">
	    	<div class="rh-cartbox widget">
	            <div>
                    ...
                    <div><i class="far fa-user"></i> <?php esc_html_e( 'Registration', 'rehub-theme' );  echo ': ' .date_i18n( get_option( "date_format" ), strtotime( $vendor->user_registered )); ?></div>
	                    ...
	                    <?php if (!empty($mycredpoint)) :?><div><i class="far fa-bar-chart"></i><?php echo esc_html($mycredlabel); ?>: <?php echo esc_attr($mycredpoint); ?> </div><?php endif; ?>
	                </div>
					<?php if ( class_exists( 'WCVendors_Pro' ) ) : ?>
						<div class="profile-description">
							<div>
								<span><?php esc_html_e( 'Contact', 'rehub-theme' ); ?></span>
								<p>
								<?php if( !$active_address_and_map ) : ?>
								...
								<?php if( !$active_contact_widget ) : ?>
									<?php if ($phone):?>
										<a href="tel:<?php echo esc_attr($phone); ?>"><i class="fas fa-mobile-android-alt"></i> <?php echo esc_html($phone); ?></a><br />
									<?php endif;?>
									<?php if ($company_url):?>
										<a href="<?php echo esc_url($company_url); ?>" target="_blank" rel="nofollow"><i class="far fa-globe"></i> <?php echo esc_url($company_url); ?></a>
									<?php endif;?>
								<?php endif; ?>
								</p>
							</div>
						...
						<?php if ($social_icons):?>
			                <div class="profile-socbutton">
			                    <div class="social_icon small_i">
				                    <?php if ( $facebook_url != '') { ?><a href="<?php echo esc_url($facebook_url); ?>" target="_blank" class="author-social fb" rel="nofollow"><i class="fab fa-facebook"></i></a><?php } ?>
				                    <?php if ( $instagram_username != '') { ?><a href="//instagram.com/<?php echo esc_attr($instagram_username); ?>" target="_blank" class="author-social fb" rel="nofollow"><i class="fab fa-instagram"></i></a><?php } ?>
				                    <?php if ( $twitter_username != '') { ?><a href="//twitter.com/<?php echo esc_attr($twitter_username); ?>" target="_blank" class="author-social tw" rel="nofollow"><i class="fab fa-twitter"></i></a><?php } ?>
				                    <?php if ( $googleplus_url != '') { ?><a href="<?php echo esc_url($googleplus_url); ?>" target="_blank" class="author-social gp" rel="nofollow"><i class="fab fa-google-plus"></i></a><?php } ?>
				                    <?php if ( $pinterest_url != '') { ?><a href="<?php echo esc_url($pinterest_url); ?>" target="_blank" class="author-social gp" rel="nofollow"><i class="fab fa-pinterest"></i></a><?php } ?>
				                    <?php if ( $youtube_url != '') { ?><a href="<?php echo esc_url($youtube_url); ?>" target="_blank" class="author-social yt" rel="nofollow"><i class="fab fa-youtube"></i></a><?php } ?>
				                    <?php if ( $linkedin_url != '') { ?><a href="<?php echo esc_url($linkedin_url); ?>" target="_blank" class="author-social fb" rel="nofollow"><i class="fab fa-linkedin"></i></a><?php } ?>
			                     </div>
			                </div>
		           		<?php endif;?>
		           	<?php else:?>
						<div class="profile-description">
							<div>
								...
								<p>
									...
									<?php endif; ?>
									<?php if ($wcfreephone):?>
										<a href="tel:<?php echo esc_html($wcfreephone); ?>"><i class="fas fa-mobile-android-alt"></i> <?php echo esc_html($wcfreephone); ?></a>
									<?php endif;?>
								</p>
							</div>
						</div>		           		
	            ...
	            <?php if ( !empty( $vendor->description ) ) : ?>
	                <div class="profile-description">
	                    <div>
	                        ...
	                        <p><?php echo wp_kses_data($vendor->description); ?></p>
	                    </div>
	                </div>
	            <?php endif; ?>
                    ...
                    <div class="profile-usermenu mt20">
	                    <ul class="user-menu-tab" role="tablist">
	                        <li class="text-center">
	                            <a href="<?php echo bp_core_get_user_domain( $vendor_id ); ?>"><i class="far fa-folder-open"></i><?php esc_html_e( 'Show full profile', 'rehub-theme' ); ?></a>
	                        </li>
	                    </ul>
                    </div>
                <?php endif; ?>
