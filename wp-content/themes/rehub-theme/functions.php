...
add_filter('woocommerce_checkout_fields', 'prime_checkout_fields', 100, 1);
...
function prime_get_edit_user_id() {
    return isset( $_GET['user_id'] ) ? (int) $_GET['user_id'] : get_current_user_id();
}

function prime_is_userdata( $key ) {
    $userdata = array(
        'user_pass',
        'user_login',
        'user_nicename',
...
function prime_get_userdata( $user_id, $key ) {
    
    
    if ( ! prime_is_userdata( $key ) ) {
...
function prime_checkout_fields($fields){
    ...
    $address = $fields['additional'];
    $address['additional_wooccm2']['value']= prime_get_userdata(prime_get_edit_user_id(), 'additional_wooccm2'); //ruc 
    $address['additional_wooccm1']['value']= prime_get_userdata(prime_get_edit_user_id(), 'additional_wooccm1');//razon social
    
    $fields['additional']=$address;

    $fields['billing']['billing_address_1']['label']='Dirección (En caso de pedir factura ingrese Dirección Fiscal)';
    $fields['billing']['billing_address_1']['priority']=115;
...
function save_custom_field_usermeta($order_id){
    ...
    if ( isset( $_POST['shipping_shipperagency'] ) ) {
        update_post_meta($order_id, 'shipping_shipperagency', $_POST['shipping_shipperagency']);
    }

	if( isset( $_POST['additional_wooccm2'] ) ) {
	    update_user_meta( prime_get_edit_user_id(), 'additional_wooccm2', sanitize_text_field( $_POST['additional_wooccm2'] ) );
	}

	if( isset( $_POST['additional_wooccm1'] ) ) {
	    update_user_meta( prime_get_edit_user_id(), 'additional_wooccm1', sanitize_text_field( $_POST['additional_wooccm1'] ) );
	}
...
{
	$items = array();
	$items['SIN DESPACHO'] = 'SIN DESPACHO';
    	// DESHABILITADO TEMPORALMENTE... SOLO DESPACHOS PARA TRUJILLO Y LIMA
        /*
...
function custom_validate_fields( $fields, $errors ){
    ...
    if($fields['billing_wooccm9']=='RUC' && strlen($fields['billing_wooccm10'])!=11){//DNI
        $errors->add( 'validation', count($fields['billing_wooccm10']).'RUC debe tener 11 caracteres numéricos.' );
    }
...
function syncronize_order_to_erp( $orderid){
    ...
    if(isset($syncro) && $syncro=='OK')
        ...
        $lines_string = "";
        if(!$alreadySync){
            $url = 'https://clute.com.pe/ecommerce/index.php/woocommerce/woocommerce_addorder/5245928cww323dm'.$orderid;
            ...
            if($lines_string=='1'){
               ...
               /* $url = 'http://clute.com.pe/wordpress/ecommerce/index.php/woocommerce/woocommerce_updatestocks/5245928cww323dm';
                $handle=fopen($url,"rb");
                fclose($handle);*/
...
function tienda_login_redirect( $redirect='shop', $user=null ) {
...
add_filter( 'woocommerce_login_redirect', 'tienda_login_redirect' );
...
//////CODIGO PARA OCULTAR PRECIOS Y CARRITO CUANDO NO ESTAS REGISTRADO//////
...
else return "";//return '<a href="' .get_permalink(woocommerce_get_page_id('myaccount')). '">Login</a> or <a href="'.site_url('/wp-login.php?action=register&redirect_to=' . get_permalink()).'">Register</a> to see price!';
}
...
if (!is_user_logged_in() ) {      
 ...
 remove_action( 'woocommerce_external_add_to_cart', 'rh_woo_external_add_to_cart', 30 );
...
//remove_action( 'woocommerce_checkout_after_order_review', 'rehub_woo_after_checkout' );
//remove_action( 'woocommerce_before_add_to_cart_button', 'rehub_woo_countdown' );
//remove_action( 'woocommerce_product_query', 'rh_change_product_query', 99 ); //Here we change and extend product loop data
remove_action('rhwoo_template_single_add_to_cart', 'woocommerce_template_single_add_to_cart');

 remove_action( 'woocommerce_external_add_to_cart', 'rh_woo_external_add_to_cart', 10 );
 remove_action( 'woocommerce_checkout_before_customer_details', 'woocommerce_template_single_add_to_cart', 30 );
...
add_filter( 'rest_authentication_errors', function( $result ) {
  ...
  if ( ! is_user_logged_in() ) {
    return new WP_Error( 'rest_not_logged_in', 'Usuario no está en sesión.', array( 'status' => 401 ) );
  }
  return $result;
});*/
