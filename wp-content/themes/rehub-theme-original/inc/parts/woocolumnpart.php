...
<?php $custom_image_size = rehub_option('wooloop_image_size');?>
<?php if($custom_image_size):?>
    <?php $size_array = explode('-', $custom_image_size);?>
    <?php if(is_array($size_array)):?>
        <?php $custom_img_width = trim($size_array[0]);?>
        <?php $custom_img_height = trim($size_array[1]);?>
        <?php $custom_col = true;?>
    <?php endif;?>
<?php endif;?>
        ...
        <div class="woo_column_btn text-center">   
            ...
            <?php elseif ( $product->add_to_cart_url() !='') : ?>               
                <?php  echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                        sprintf( '<a href="%s" data-product_id="%s" data-product_sku="%s" class="re_track_btn fontbold rehub-main-font rehub-main-color upper-text-trans %s %s product_type_%s"%s %s>%s</a>',
