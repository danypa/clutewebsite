...
class WC_Address_Book {
	...
	public function update_address_names( $user_id, $name ) {
		...
		$this->add_address_name( $user_id, $name ); // Original...
		//$this->add_address_name( $user_id, $name .'@1'. $_POST['DeptoCode_shipping'] .'@2'. $_POST['shipping_City'] .'@3'. $_POST['shipping_DistCode'] ); // Cambio...
	...
	private function add_address_name( $user_id, $name ) {
		...
		if ( ! in_array( $name, $address_names, true ) ) {
			array_push( $address_names, $name ); //pruebas.... . '@1Lima@2Lima@3Los Olivos' 
			$this->save_address_names( $user_id, $address_names ); // original...
			//$this->save_address_names( $user_id, $address_names . '@1' . 'Lima' . '@2' . 'Lima' . '@1' . 'Los Olivos'); // cambio...
			//echo $name.'...'.json_encode($addresses_names);
		}
	}
	...
	public function get_address_names( $user_id = null ) {
		...
		if ( empty( $address_names ) ) {
		    ...
		    $shipping_address = get_user_meta( $user_id, 'shipping_calle', true );//BY PALK
	...
	public function get_address_book( $user_id = null ) {
		...
		//echo json_encode($address_names);
		...
		if ( is_user_logged_in() ) {
			...
			if ( ! empty( $address_book ) && false !== $address_book ) {
			    ...
			    foreach ( $address_book as $name => $address ) {
					if ( ! empty( $address[ $name . '_calle' ] ) ) {//BY PALK cambiamos address_1 por calle 
...
/*
	...
	public function wc_address_book_delete( $address_name ) {
		...
		//$address_ubigeo = $this ->get_address_ubigeo ( $customer_id ); // CAMBIOS...
	...
	public function wc_address_book_checkout_update() {
		...
		if ( 'add_new' !== $name ) {
			...
			foreach ( $address_book[ $name ] as $field => $value ) {
				...
				//replace BY PALK
				if($field=='shipping_department')
				    $departamentoShipping =  $value ;
				    if($field=='shipping_city')
				        $provinciaShipping = $value;
				        if($field=='shipping_district')
				            $distritoShipping =$value;
				        
			}
			
			$response['dptos_anterior'] = $departamentoShipping;
			$response['provs_anterior'] = $provinciaShipping;
			$response['dists_anterior'] = $distritoShipping;
