...
class PluginUpdateChecker_3_2 {
	 ...
	 *
	 * Has chsnges from 4.5 ver. to combne with PHP 7.3 // @Berdych
	 *
	 * @return bool
	 */
	protected function isMuPlugin() {
		...
		if ( $cachedResult === null ) {
			...
			if (($muPluginDir === false) || ($pluginPath === false)) {
				$muPluginDir = $this->normalizePath(WPMU_PLUGIN_DIR);
				$pluginPath  = $this->normalizePath($this->pluginAbsolutePath);
			}

			$cachedResult = ($muPluginDir && strpos($pluginPath, (string) $muPluginDir) === 0);
		}
...
$update_checker = PucFactory::buildUpdateChecker(
	RHWCT_REPO .'?action=get_metadata&slug='. RHWCT_SLUG,
	RHWCT_FILE,
	RHWCT_REPO,
	'24'
);
