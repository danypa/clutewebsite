...
function wc_add_to_menu_ur( $items ) {

    $items = array('edit-password' => __( 'Change Password', 'user-registration' )) + $items;
    $items = array('' => __( 'Dashboard', 'user-registration' )) + $items;
    
    return $items;
}

add_filter( 'woocommerce_account_menu_items', 'wc_add_to_menu_ur' );


add_action( 'init', 'add_endpoint_ur' );
add_filter( 'query_vars', 'wc_ur_query_vars', 0 );
add_action( 'woocommerce_account_edit-profile_endpoint', 'wc_ur_edit_profile_content' );
add_action( 'woocommerce_account_edit-password_endpoint', 'wc_ur_edit_password_content' );


function add_endpoint_ur(){
    add_rewrite_endpoint( 'edit-profile', EP_PAGES );
    add_rewrite_endpoint( 'edit-password', EP_PAGES );
}

function wc_ur_query_vars( $vars ) {
    $vars[] = 'edit-profile';
    $vars[] = 'edit-password';
    return $vars;
}

function wc_ur_edit_profile_content($type) {
    wc_get_template( 'myaccount/form-edit-profile.php', null, '', plugin_dir_path( dirname( __FILE__ ) ) . 'templates/' );
    
}


function wc_ur_edit_password_content($type) {
    wc_get_template( 'myaccount/form-edit-password.php', null, '', plugin_dir_path( dirname( __FILE__ ) ) . 'templates/' );
