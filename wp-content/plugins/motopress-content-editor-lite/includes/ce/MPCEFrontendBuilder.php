...
class MPCEFrontEndBuilder {
	...
	/**
	 * @var MPCEFrontEndBuilder
	 */
	private static $_instance;

	/**
	 * MPCEFrontEndBuilder constructor.
	 *
	 */
	private function __construct() {

		$this->templatePath = mpceSettings()['plugin_dir_path'] . 'templates/editor-page.php';

		$this->isEdit = isset( $_GET['mpce-edit'] ) && $_GET['mpce-edit'];

		if ($this->isEdit) {

			$this->_fixNextgenGallery();

			add_action( 'template_redirect', array( $this, 'init' ) );

		}

		$this->postLocker = new MPCEPostLocker();
	}

	/**
	 * @return MPCEFrontEndBuilder
	 */
	...
	public function init() {
		...
		MPCEContentManager::getInstance()->enablePostForEditor( $postId, true );

		// Lock Post if post is not locked
		if ( ! $this->postLocker->getLockingUser( $postId ) ) {
			$this->postLocker->lockPost( $postId );
		}
...
//		add_action( 'mpce_frontend_editor_footer', 'wp_auth_check_html' );
	...
	public function motopressCEHTML() {
		...
		$showLeftBarTitle = __('Add', 'motopress-content-editor-lite');
	...
	public function enqueueScripts() {
		...
		$lockingUser = $this->postLocker->getLockingUser($postID);
		$topEditorData['settings']['lockingUser'] = $lockingUser ? $lockingUser->display_name : false;

		$sendback = wp_get_referer();
		if ( $sendback &&
		     false === strpos($sendback, 'mpce-edit=1') &&
		     false === strpos( $sendback, 'post.php' ) &&
             false === strpos( $sendback, 'post-new.php' )
		) {
			$sendbackText = __('Go back', 'motopress-content-editor-lite');
		} else {
			...
			$sendbackText = sprintf( __( 'Go to %s', 'motopress-content-editor-lite' ), get_post_type_object( $postType )->labels->all_items );
		}
		$topEditorData['settings']['sendback'] = $sendback;
		$topEditorData['settings']['sendbackText'] = $sendbackText;

		wp_localize_script( 'mpce-top-editor', 'motopressCE', apply_filters( 'motopress_ce_localize_settings', $topEditorData, $postID ) );
	...
	/**
	 * @return MPCEPostLocker
	 */
	public function getPostLocker() {
		return $this->postLocker;
	}

	/**
	 * @param int|string $postId Optional.
	 *
	 * @return string
	 */
	public function getEditUrl( $postId = null ) {
		if ( !isset( $postId ) ) {
			$postId = get_the_ID();
		}

		return add_query_arg( 'mpce-edit', '1', get_permalink( $postId ) );
	}

	/**
	 * Disable NextGEN Gallery Resource Manager which uses output buffering
	 */
	private function _fixNextgenGallery(){
