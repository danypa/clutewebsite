...
//		}
	...
	private function enqueueScripts(){
		if (!mpceIsEditorScene()) {
			...
			if (in_array($show_content, array('full', 'excerpt'))) {
				wp_enqueue_style('mpce-font-awesome');
			}

			wp_enqueue_style('mpce-bootstrap-grid');
			MPCEShortcode::enqFrontScript();
		}
	}
	...
	public static function getFullTemplatePath($template) {
		...
		if ($isMCPETemplate) {
			...
			$fullTemplatePath = preg_replace($pattern, mpceSettings()['plugin_dir_path'], $template);
		} else {
	...
	public function render(){
        ...
        $result .= '<div class="motopress-posts-grid-obj motopress-posts-grid-gap-'. $this->getAttr('posts_gap')
