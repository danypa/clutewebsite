...
<h3 class="woocommerce-column__title" style="display: block; margin: 0px 0 8px; text-align: left; font-size: 16px; line-height: 26px; font-family: &quot;Helvetica Neue&quot;, Helvetica, Roboto, Arial, sans-serif; font-style: normal; font-weight: 500; color: #787878;">Información Adicional</h3>
<!-- vanessa -->
<table width="100%" border="1" bordercolor="#CCC" style="margin-bottom: 40px; background: #fcfcfc;"><tr><td style="border-color: #19428A;"><br><!-- vanessa -->
    <?php

	$checkout = array('billing_wooccm9'=>'Tipo de Documento','billing_wooccm10'=>'Nro. de Documento (DNI/CE)','additional_wooccm0'=>'Tipo Documento','additional_wooccm2'=>'RUC', 'additional_wooccm1'=>'Razón Social');
      foreach ($checkout as $key => $label) :

            ?>

              <?php if ($value = get_post_meta($order->id, sprintf('_%s', $key), true)): ?>
                <?php echo '<p style="font-size: 13px;padding-left:20px"><strong>'.esc_html($label).':</strong> ' . esc_html($value) . '</p>'; 
...
$calle=get_post_meta( $order->id, '_shipping_calle', true );
	...
	$agencia = get_shippingagency(getProvinciaByidProv(explode("-",$city)[0]), get_post_meta( $order->id, 'shipping_shipperagency', true ));
	echo '<p style="font-size: 13px;padding-left:20px"><strong>Agencia de Transporte:</strong> ' . esc_html($agencia) . '</p>'; 
